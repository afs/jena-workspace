...
public class RiotTests
{
    ...
    public static Runnable makeRIOTTest(ManifestEntry entry) {
        ...
        try
        {
            Resource testType = TestUtils.getResource(item, RDF.type) ;
            if ( testType == null )
            ...
            if ( testType.equals(VocabLangRDF.TestPositiveSyntaxTTL) )
                return new SyntaxLangTest(entry, RDFLanguages.TURTLE, true) ;
            if ( testType.equals(VocabLangRDF.TestNegativeSyntaxTTL) )
                return new SyntaxLangTest(entry, RDFLanguages.TURTLE, false) ;

            if ( testType.equals(VocabLangRDF.TestPositiveSyntaxTriG) )
                return new SyntaxLangTest(entry, RDFLanguages.TRIG, true) ;
            if ( testType.equals(VocabLangRDF.TestNegativeSyntaxTriG) )
                return new SyntaxLangTest(entry, RDFLanguages.TRIG, false) ;

            if ( testType.equals(VocabLangRDF.TestPositiveSyntaxNT) )
                return new SyntaxLangTest(entry, RDFLanguages.NTRIPLES, true) ;
            if ( testType.equals(VocabLangRDF.TestNegativeSyntaxNT) )
                return new SyntaxLangTest(entry, RDFLanguages.NTRIPLES, false) ;

            if ( testType.equals(VocabLangRDF.TestPositiveSyntaxNQ) )
                return new SyntaxLangTest(entry, RDFLanguages.NQUADS, true) ;
            if ( testType.equals(VocabLangRDF.TestNegativeSyntaxNQ) )
                return new SyntaxLangTest(entry, RDFLanguages.NQUADS, false) ;

            if ( testType.equals(VocabLangRDF.TestPositiveSyntaxRJ) )
                return new SyntaxLangTest(entry, RDFLanguages.RDFJSON, true) ;
            if ( testType.equals(VocabLangRDF.TestNegativeSyntaxRJ) )
                return new SyntaxLangTest(entry, RDFLanguages.RDFJSON, false) ;
