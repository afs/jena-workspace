...
public class ExecQueryTest implements Runnable {

    private final ManifestEntry testEntry;
    SPARQLResult                results; // expected.
    QueryTestItem                    testItem;

    public ExecQueryTest(ManifestEntry entry, Syntax dftSyntax) {
        testEntry = entry;
    ...
    public void run() {
        ...
        try {
            try {
                query = T.queryFromEntry(testEntry);
            } catch (QueryException qEx) {
                query = null;
                qEx.printStackTrace(System.err);
                T.setupFailure("Parse failure: " + qEx.getMessage());
                throw qEx;
            }

            Dataset dataset = setUpDataset(query, testItem);
            if ( dataset == null && !doesQueryHaveDataset(query) )
                T.setupFailure("No dataset for query");

            try (QueryExecution qe = (dataset == null)
                ? QueryExecutionFactory.create(query) : QueryExecutionFactory.create(query, dataset)) {
            ...
            T.setupFailure("IOException: " + ioEx.getMessage());
            IO.exception(ioEx);
        } catch (NullPointerException ex) {
            throw ex;
        } catch (Exception ex) {
            ex.printStackTrace(System.err);
            T.setupFailure("Exception: " + ex.getClass().getName() + ": " + ex.getMessage());
        }
    }
    ...
    private Dataset setUpDataset(Query query, QueryTestItem testItem) {
        try {
            ...
            if ( !doesQueryHaveDataset(query) )
                T.setupFailure("No dataset");

            // Left to query
            return null;

        } catch (JenaException jEx) {
            T.setupFailure("JenaException creating data source: " + jEx.getMessage());
            return null;
        }
    ...
    private void compareGraphResults(Model resultsActual, Query query) {
        if ( results != null ) {
            try {
                if ( !results.isGraph() )
                    T.testFailure("Expected results are not a graph: " + testItem.getName());
                ...
                T.testFailure("Exception in result testing (" + typeName + "): " + ex);
            }
        }
    }
