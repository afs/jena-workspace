...
// mainHttps();
    ...
    public static void mainServer() throws IOException {
        ...
        FusekiLib.modifyForAccessCtl(server.getDataAccessPointRegistry(), DataAccessCtl.requestUserServlet);
        // DEV
...
// });
    ...
    public static FusekiServer fusekiServer(int port) {
        //FusekiServer server = FusekiLib.fusekiBuilder/*AccessCtl*/(DataAccessCtl.requestUserServlet).port(port).parseConfigFile("config-access-all.ttl")
        FusekiServer server = FusekiLib.fusekiBuilderAccessCtl(DataAccessCtl.requestUserServlet).port(port).parseConfigFile("config-access-all.ttl")
            .build();
        server.start();
        return server;
    }
    ...
    public static FusekiServer fusekiServerProg(int port) {
        ...
        //FusekiServer server = FusekiLib.fusekiBuilder/*AccessCtl*/(DataAccessCtl.requestUserServlet)
        FusekiServer server = FusekiLib.fusekiBuilderAccessCtl(DataAccessCtl.requestUserServlet)
