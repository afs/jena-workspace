...
public abstract class ActionService extends ActionBase {
    ...
    /**
     * ActionService for a directly called servlet. The request handler will be
    ...
    protected void execCommonWorker(HttpAction action) {
        ...
        //   (includes anything view FusekiFilter)
        // For directly called services, it is the service code itself.
        ActionService handler = selectProcessor(action, operation);
        if ( handler == null )
    ...
    /** Default implementation of {@code selectProcessor} which returns "this". */ 
