...
public class Transformer2
{
    ...
    public static Op transformSkipService(Transform transform, ExprTransform exprTransform, Op op,
        ...
        if ( true ) {
            ...
            // transformation).
            Transform walker = new TransformSkipService(transform) ;
            return Transformer2.transform(walker, exprTransform, op, beforeVisitor, afterVisitor) ;
        } else {
            ApplyTransformVisitorServiceAsLeaf v = new ApplyTransformVisitorServiceAsLeaf(transform, exprTransform) ;
            WalkerVisitorSkipService walker = new WalkerVisitorSkipService(v, beforeVisitor, afterVisitor) ;
            OpWalker.walk(walker, op) ;
    ...
    class ApplyTransformVisitor extends OpVisitorByType {
        ...
        private ExprList transform(ExprList exprList, ExprTransform exprTransform) {
            if ( exprList == null || exprTransform == null )
                return exprList ;
            return ExprTransformer2.transform(exprTransform, opTransform, exprList) ;
        }
        ...
        private VarExprList process(VarExprList varExpr, ExprTransform exprTransform) {
        ...
        private ExprList process(ExprList exprList, ExprTransform exprTransform) {
            if ( exprList == null )
                return null ;
        ...
        private Expr process(Expr expr, ExprTransform exprTransform) {
            Expr e = expr ;
            Expr e2 = e ;
            if ( e != null )
                e2 = transform(e, exprTransform) ;
        ...
        protected void visit1(Op1 op) {
            ...
            if ( op.getSubOp() != null )
                subOp = pop() ;
            push(op.apply(opTransform, subOp)) ;
        }
        ...
        protected void visitFilter(OpFilter opFilter) {
            ...
            push(f.apply(opTransform, subOp)) ;
        }
