        ...
        if ( nv.isNumber() ) {
            if ( castType.equals(XSDDatatype.XSDdecimal) ) {   
                ...
                if ( isDouble(nv) || isFloat(nv) ) {
                    ...
                    // **** Use lexical form if possible.
                    // And updated doubleToDecimalString
                    // Breaks TestCastXSD.cast_decimal_16
                    // @Test public void cast_decimal_16()
                    //    { testCast      ("xsd:decimal('1'^^xsd:double)",        "'1.0'^^xsd:decimal") ; }
  
                    
                    // Casting to decimal - any numeric double or flat lexical for can be used directly.
                    String lex = null;
                    if ( nv.getNode() != null ) {
                        String x = nv.getNode().getLiteralLexicalForm();
                        if ( !x.contains("e") && !x.contains("E") )
                            lex = nv.getNode().getLiteralLexicalForm();
                    }
                    if ( lex == null )
                        lex = doubleToDecimalString(d) ;
