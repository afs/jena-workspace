...
public class BuilderSecondaryIndexesParallel_tdb1 implements BuilderSecondaryIndexes_tdb1
{
    private LoadMonitor_tdb1 monitor ;

    public BuilderSecondaryIndexesParallel_tdb1(LoadMonitor_tdb1 monitor) { this.monitor = monitor ; } 
    ...
    private Runnable setup(final Semaphore sema, final TupleIndex srcIndex, final TupleIndex destIndex, final String label)
    {
        Runnable builder = new Runnable(){
            ...
            public void run()
            {
                LoaderNodeTupleTable_tdb1.copyIndex(srcIndex.all(), new TupleIndex[]{destIndex}, label, monitor) ;
                sema.release() ;
            }} ;
