...
import tdb2.loader.parallel.Indexer;

public class DevLoaderDataInput1 {
    ...
    public static void dwim1(DatasetGraph dsg, String datafile) {
        ...
        //Destination<Tuple<NodeId>> functionDevNull = (x)->{};
        //Destination<Tuple<NodeId>> functionPrint = (x)->System.out.println(x);
        
        // Prefix handler.
        //functionPrefix
        // Should this be a two stage process?
        DataInput inputStage = new DataInput(dsgtdb, functionIndexer, output);

        // Minimum is DataInput+Index to SPO/GSPO.
        
        // Or this thread
        long time = TimerX.time(()->{
            inputStage.startBulk();
            // Drive data into the input stage.
            RDFDataMgr.parse(inputStage, datafile);
            inputStage.finishBulk();
            // Wait.
            indexer.waitFinish();
        });
