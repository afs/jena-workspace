...
public class QueryIterHashJoin2 extends AbstractIterHashJoin {
    ...
    public static QueryIterator create(JoinKey joinKey, QueryIterator left, QueryIterator right, ExecutionContext execCxt) {
        ...
        if ( joinKey != null && joinKey.length() > 1 )
            Log.warn(QueryIterHashJoin2.class, "Multivariable join key") ; 
        return new QueryIterHashJoin2(joinKey, left, right, execCxt) ; 
    }
