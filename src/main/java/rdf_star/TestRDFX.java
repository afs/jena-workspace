...
public class TestRDFX {
    ...
    @Test public void rdfx_basic() {
        ...
        Graph g2 = RDFX.encodeRDFStar(g1);
    ...
    private void testEncodeDecode(String str) {
        ...
        Graph g1 = RDFX.encodeRDFStar(graph);
        testNoTripleTerms(g1);
        Graph g2 = RDFX.decodeRDFStar(g1);
        // check for no triple terms.
        boolean b = graph.isIsomorphicWith(g2);
        if ( ! b ) {
    ...
    private void testNoTripleTerms(Graph graph) {
        assertFalse(G.find(graph, null, null, null).filterKeep(RDFX::tripleHasNodeTriple).hasNext());
    }
    ...
    private void testInPlace(String str) {
        ...
        Graph g1a = RDFX.encodeRDFStar(graph);
        Graph g1 = RDFX.encodeRDFStarInPlace(graph);
        ...
        boolean b = g1.isIsomorphicWith(g1a);
        if ( ! b ) {
            System.out.println("-- Mismatch");
